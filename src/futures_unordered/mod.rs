...
pub struct FuturesUnordered<K: Hash + Eq, Fut> {
    ready_to_run_queue: Arc<ReadyToRunQueue<K, Fut>>,
    head_all: AtomicPtr<Task<K, Fut>>,
    is_terminated: AtomicBool,
}

unsafe impl<K: Hash + Eq, Fut: Send> Send for FuturesUnordered<K, Fut> {}
unsafe impl<K: Hash + Eq, Fut: Sync> Sync for FuturesUnordered<K, Fut> {}
impl<K: Hash + Eq, Fut> Unpin for FuturesUnordered<K, Fut> {}

impl<K: Hash + Eq> Spawn for FuturesUnordered<K, FutureObj<'_, ()>> {
...
impl<K: Hash + Eq> LocalSpawn for FuturesUnordered<K, LocalFutureObj<'_, ()>> {
...
impl<K: Hash + Eq, Fut> Default for FuturesUnordered<K, Fut> {
    fn default() -> Self {
        Self::new()
    }
}

impl<K: Hash + Eq, Fut> FuturesUnordered<K, Fut> {
    ...
    fn link(&self, task: Arc<Task<K, Fut>>) -> *const Task<K, Fut> {
...
impl<K: Hash + Eq, Fut: Future> Stream for FuturesUnordered<K, Fut> {
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        loop {
            ...
            struct Bomb<'a, K: Hash + Eq, Fut> {
                queue: &'a mut FuturesUnordered<K, Fut>,
...
impl<K: Hash + Eq, Fut> Debug for FuturesUnordered<K, Fut> {
...
impl<K: Hash + Eq, Fut> Drop for FuturesUnordered<K, Fut> {
    fn drop(&mut self) {
...
impl<'a, K: Hash + Eq, Fut: Unpin> IntoIterator for &'a FuturesUnordered<K, Fut> {
    type Item = &'a Fut;
...
impl<'a, K: Hash + Eq, Fut: Unpin> IntoIterator for &'a mut FuturesUnordered<K, Fut> {
    type Item = &'a mut Fut;
...
impl<K: Hash + Eq, Fut: Unpin> IntoIterator for FuturesUnordered<K, Fut> {
    type Item = Fut;
...
impl<K: Hash + Eq, Fut> FromIterator<Fut> for FuturesUnordered<K, Fut> {
...
impl<K: Hash + Eq, Fut: Future> FusedStream for FuturesUnordered<K, Fut> {
    fn is_terminated(&self) -> bool {
        self.is_terminated.load(Relaxed)
    }
}

impl<K: Hash + Eq, Fut> Extend<Fut> for FuturesUnordered<K, Fut> {
